/**
 * E2E tests for post flows: create, delete via options sheet, report, bookmark
 */
...
let extraCleanup: Array<{ user: TestUser; token: string }> = [];

/** Log in via UI and return the auth token from localStorage.
 *  Also calls POST /api/auth/sync so the backend user record exists
 *  before any API calls (avoids owner_id FK race condition). */
...
test.afterEach(async ({ page }) => {
    // Always clean up extra users created within the test (author users etc.)
    // This runs even when assertions fail mid-test.
    for (const { user, token } of extraCleanup) {
        try {
...
test.describe('Post options sheet', () => {
    ...
    test("post options sheet shows Report for another user's post", async ({ page }) => {
        ...
        // Register for cleanup — runs in afterEach even if assertions fail
        extraCleanup.push({ user: author, token: authorToken });
